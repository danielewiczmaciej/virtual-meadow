...
#define ADD(X,Y) world->add_organism(new Wolfberries(world, { pos.x+X, pos.y+Y}), { pos.x+X, pos.y +Y })

Wolfberries::Wolfberries() {}

Wolfberries::Wolfberries(World* world, const CORDS pos) {
	this->world = world;
	this->pos = pos;
	this->ini = 0;
	this->pow = 99;
...
Wolfberries::Wolfberries(World* world, const CORDS pos, int age, int ini, int pow) {
	this->world = world;
	this->pos = pos;
	this->ini = ini;
	this->age = age;
...
const void Wolfberries::draw() { cout << WOLF_BERRIES; }

const char Wolfberries::get_sign() { return WOLF_BERRIES; }

void Wolfberries::reproduce() {
...
void Wolfberries::collision(Org* collider) {
	string temp = "";
